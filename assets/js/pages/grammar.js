/**
 * EnglishPath — Grammar A1–A2 Page
 * Fase 6 — v0.7.0
 * IIFE module pattern
 */
...
(function GrammarPage() {
  ...
  Router.guard();
  App.init('foundation-grammar');

  const user = Auth.getActiveUser();
  const STORAGE_KEY = 'grammar_foundation';

  // ── State ─────────────────────────────────────────────────
  let currentTopicId = null;
  let currentMode    = 'study'; // 'study' | 'quiz'
  let currentLevel   = 'all';   // 'all' | 'A1' | 'A2'
  ...
  let reorderBank    = []; // original word order (indices)
  ...
  function getDefaultStats() {
    return {
      topicsStudied: {},   // { topicId: true } — opened study tab
      quizResults:   {},   // { topicId: { best: N, attempts: N, lastScore: N } }
      totalXP:       0,
      quizzesDone:   0,
    };
  }

  function getTopicProgress(topicId, stats) {
    const r = stats.quizResults[topicId];
    if (!r) return 0;
    // progress = best score percentage
    return r.best;
  }
      ...
      return `
        <div class="topic-progress-row" data-topic-id="${t.id}">
          ...
          <div class="topic-progress-info">
            <div class="topic-progress-name">${t.title}</div>
  ...
  function renderStudyContent(topic) {
    ...
    container.innerHTML = topic.sections.map((sec, i) => {
      const examplesHTML = sec.examples.map(ex => {
        // Extract spoken text (remove everything after " (")
        const spokenText = ex.en.split(' (')[0].split(' →')[0].trim();
        return `
          <li class="example-item">
            ...
            <button class="speak-mini-btn" data-speak="${escapeAttr(spokenText)}">🔊</button>
          </li>
        `;
      }).join('');
    ...
    container.querySelectorAll('[data-speak]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        // Only speak if it's actual English words (not meta-text)
        speak(text);
      });
    });
  }
  ...
  function resetQuizUI() {
    ...
    const topic = GRAMMAR_DATA.getTopic(currentTopicId);
    if (topic) {
  ...
  function startQuiz() {
    ...
    quizQuestions = GRAMMAR_DATA.getQuiz(currentTopicId, 10);
    quizIndex     = 0;
    quizScore     = 0;
    quizAnswered  = false;
  ...
  function renderFill(q) {
    ...
    input.onkeydown = (e) => {
      if (e.key === 'Enter' && !quizAnswered) checkFill(q.answer);
    };
  }
  ...
  function renderReorder(q) {
    ...
    function placeWord(chip, word, idx) {
      ...
      placed.addEventListener('click', () => removeWord(placed, chip, idx));
      answerArea.appendChild(placed);
    }

    function removeWord(placed, chip, idx) {
      if (quizAnswered) return;
  ...
  function showFeedback(isCorrect, correctAnswer) {
    ...
    if (isCorrect) {
      ...
      feedback.textContent = '✅ Benar! Bagus sekali!';
    } else {
      feedback.classList.add('wrong-feedback');
      feedback.textContent = `❌ Kurang tepat. Jawaban yang benar: "${correctAnswer}"`;
    }
  }
  ...
  function showResult() {
    ...
    if (typeof XPSystem !== 'undefined') {
      const topic = GRAMMAR_DATA.getTopic(currentTopicId);
      XPSystem.addXP(xp, 'quiz', `Grammar quiz: ${topic ? topic.title : ''}`);
    }
    ...
    // Badge: grammar_pro if best score 10/10 on any topic
  ...
  function getResultLabel(pct) {
    if (pct === 100) return 'Sempurna! Kamu sudah menguasai topik ini!';
  ...
  function bindEvents() {
    ...
    document.getElementById('btn-check').addEventListener('click', () => {
      ...
      const q = quizQuestions[quizIndex];
      if (q.type === 'mcq') {
        // MCQ is handled by option click; if not clicked, remind user
    ...
    // Fill input Enter key already bound in renderFill
  ...
  function init() {
    ...
    document.getElementById('hdr-total-quiz').textContent = `${totalQuiz}+ soal latihan`;
  }
