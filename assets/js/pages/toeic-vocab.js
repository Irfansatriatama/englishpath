/**
 * EnglishPath — TOEIC Vocabulary Module
 * Fase 14a: Browse, Flashcard, SRS Review, Quiz modes
 * localStorage: ep_user_{id}_srs_toeic_vocab, ep_user_{id}_toeic_vocab
 */
const TOEICVocab = (() => {
  ...
  function _getVocabKey() { return 'toeic_vocab'; }
  function _getSRSKey()   { return 'srs_toeic_vocab'; }
  ...
  function _buildWordList() {
    ...
    if (_state.domain === 'all') {
      words = TOEICVocabData.getAllWords();
    } else {
      const dom = TOEICVocabData.getDomain(_state.domain);
  ...
  function _getStats() {
    const prog = _getProgress();
    const total = TOEICVocabData.getTotalCount();
  ...
  function _renderDomainGrid() {
    ...
    if (!container) return;
    const domains = TOEICVocabData.getDomains();
    container.innerHTML = domains.map(d => {
      ...
      return `
        <div class="toeic-domain-card ${isActive}" data-domain="${d.id}">
          <div class="toeic-domain-top">
            <div class="toeic-domain-icon">${d.icon}</div>
            <div>
              <div class="toeic-domain-name">${d.nameID}</div>
            </div>
          </div>
          <div class="toeic-domain-progress">
            <div class="toeic-domain-bar" style="width:${pct}%"></div>
          </div>
          <div class="toeic-domain-meta">
    ...
    const allCard = `
      <div class="toeic-domain-card ${_state.domain === 'all' ? 'active' : ''}" data-domain="all">
        <div class="toeic-domain-top">
          <div class="toeic-domain-icon">📚</div>
          <div><div class="toeic-domain-name">Semua Kategori</div></div>
        </div>
        <div class="toeic-domain-progress">
          <div class="toeic-domain-bar" style="width:${Math.round((Object.keys(prog).length / TOEICVocabData.getTotalCount()) * 100)}%"></div>
        </div>
        <div class="toeic-domain-meta">
          <span>${TOEICVocabData.getTotalCount()} kata</span>
          <span>${Math.round((Object.keys(prog).length / TOEICVocabData.getTotalCount()) * 100)}% selesai</span>
        </div>
      </div>`;
    container.innerHTML = allCard + container.innerHTML;

    container.querySelectorAll('.toeic-domain-card').forEach(card => {
  ...
  function _renderWordList() {
    ...
    container.innerHTML = _state.words.map(w => {
      ...
      return `
        <div class="toeic-word-item" data-word-id="${w.id}">
          <div class="toeic-word-status ${statusClass}"></div>
          <div class="toeic-word-main">
            <div class="toeic-word-text">${w.word}</div>
            <div class="toeic-word-ipa">${w.ipa}</div>
          </div>
          <div class="toeic-word-trans">${w.translation}</div>
          <button class="toeic-word-audio" data-speak="${w.word}">🔊</button>
        </div>`;
    }).join('');

    container.querySelectorAll('.toeic-word-item').forEach(item => {
      item.addEventListener('click', e => {
        if (e.target.closest('.toeic-word-audio')) return;
  ...
  function _openWordModal(wordId) {
    const w = TOEICVocabData.getWordById(wordId);
    if (!w) return;
    _markLearned(wordId);
  ...
  function _initFlashcard() {
    ...
    if (domain === 'all') {
      ...
      const d = TOEICVocabData.getDomain(domain);
  ...
  function _showFcComplete() {
    ...
    area.innerHTML = `
      <div style="text-align:center;padding:2rem;">
        ...
        <div style="color:var(--text-secondary);margin-bottom:1.5rem;">Kamu telah meninjau ${_state.fc.queue.length} kata.</div>
        <button class="toeic-fc-btn flip" id="fc-restart-btn">Mulai Lagi</button>
      </div>`;
  ...
  function _initQuiz() {
    ...
    const domain = domSel ? domSel.value : 'all';
    const questions = TOEICVocabData.buildQuiz(domain, 10);
  ...
  function _renderQuizQuestion() {
    ...
    if (optContainer) {
      optContainer.innerHTML = q.options.map((opt, i) =>
        `<button class="toeic-quiz-option" data-opt="${opt}">${opt}</button>`
      ).join('');
      optContainer.querySelectorAll('.toeic-quiz-option').forEach(btn => {
  ...
  function _answerQuiz(selected) {
    ...
    // Update buttons
    document.querySelectorAll('.toeic-quiz-option').forEach(btn => {
      btn.disabled = true;
  ...
  function _renderQuizResult() {
    ...
    if (resultEl) {
      resultEl.innerHTML = `
        <div class="toeic-quiz-result-emoji">${emojis}</div>
        <div class="toeic-quiz-result-title">${pct === 100 ? 'Sempurna!' : pct >= 80 ? 'Bagus Sekali!' : pct >= 60 ? 'Cukup Baik!' : 'Perlu Latihan Lagi'}</div>
        <div class="toeic-quiz-result-score">${score} / ${questions.length}</div>
        <div class="toeic-quiz-result-xp">+${score * 3}${pct === 100 ? ' + 20 bonus' : ''} XP</div>
        <div>
          <button class="toeic-quiz-result-btn" id="quiz-retry-btn">🔄 Coba Lagi</button>
          <button class="toeic-quiz-result-btn" onclick="TOEICVocab.switchMode('browse')">📚 Browse Kata</button>
        </div>`;
  ...
  function _initSRS() {
    ...
    const cards = dueIds.map(id => TOEICVocabData.getWordById(id)).filter(Boolean);
  ...
  function _renderSRSEmpty(msg) {
    ...
    if (!area) return;
    area.innerHTML = `
      <div class="toeic-srs-empty">
        <div class="toeic-srs-empty-icon">✅</div>
  ...
  function _renderSRSCard() {
    ...
    area.innerHTML = `
      ...
      <div class="toeic-srs-card">
        <div class="toeic-srs-word">${w.word}</div>
        <div class="toeic-srs-ipa">${w.ipa}</div>
        <button class="toeic-srs-reveal-btn" id="srs-reveal-btn">🔍 Tampilkan Jawaban</button>
        <div class="toeic-srs-answer ${revealed ? 'show' : ''}" id="srs-answer">
          <div class="toeic-srs-translation">${w.translation}</div>
          <div class="toeic-srs-example">"${w.example}"</div>
          <div class="toeic-srs-buttons">
            <button class="toeic-fc-btn hard" id="srs-hard-btn">😓 Susah</button>
            <button class="toeic-fc-btn okay" id="srs-okay-btn">🤔 Ingat</button>
            <button class="toeic-fc-btn easy" id="srs-easy-btn">😊 Mudah</button>
          </div>
        </div>
      </div>`;
  ...
  function _switchMode(mode) {
    _state.mode = mode;
    // Update tabs
    document.querySelectorAll('.toeic-vocab-tab').forEach(t => {
  ...
  function init() {
    ...
    // Mode tabs
    document.querySelectorAll('.toeic-vocab-tab').forEach(tab => {
    ...
    // Flashcard: domain & count selects, flip, srs buttons
...
document.addEventListener('DOMContentLoaded', TOEICVocab.init);
